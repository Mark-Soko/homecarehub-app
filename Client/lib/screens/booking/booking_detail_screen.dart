...
class _BookingDetailScreenState extends State<BookingDetailScreen> with WidgetsBindingObserver {
  ...
  Widget _action({required BookingDetailResponse bookingResponse}) {
    if ((bookingResponse.service != null && bookingResponse.service!.isAdvancePayment && !bookingResponse.service!.isFreeService && bookingResponse.service!.isFixedService && bookingResponse.bookingDetail!.bookingPackage == null) &&
      ...
      final showRefund = bookingResponse.bookingDetail!.paymentStatus == SERVICE_PAYMENT_STATUS_PAID ||
      ...
      return Column(
        ...
        children: [
          ...
          if (showRefund) ...[8.height, _buildRequestRefundButton(bookingResponse)],
        ],
      );
      ...
      return Column(
        ...
        !bookingResponse.bookingDetail!.isFreeService &&
        (bookingResponse.bookingDetail!.paymentStatus == SERVICE_PAYMENT_STATUS_PAID ||
            bookingResponse.bookingDetail!.paymentStatus == SERVICE_PAYMENT_STATUS_ADVANCE_PAID)) {
      return _buildRequestRefundButton(bookingResponse);
    }

    return Offstage();
  }

  Widget _buildRequestRefundButton(BookingDetailResponse bookingResponse) {
    return AppButton(
      text: 'Request Refund',
      textColor: Colors.white,
      color: Colors.orange,
      onTap: () async {
        ...
        final submitted = await showInDialog(
          context,
          barrierDismissible: false,
          builder: (ctx) => AppCommonDialog(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Your request will be reviewed by the administrator. You can add a reason below.', style: secondaryTextStyle()),
                12.height,
                AppTextField(
                  controller: reasonCont,
                  textFieldType: TextFieldType.MULTILINE,
                  minLines: 2,
                  decoration: inputDecoration(context, hintText: 'Reason (optional)'),
                ),
                16.height,
                Row(
                  mainAxisAlignment: MainAxisAlignment.end,
                  children: [
                    AppButton(text: language.lblCancel, color: context.cardColor, textColor: textPrimaryColorGlobal, onTap: () => finish(ctx, false)),
                    12.width,
                    AppButton(
                      text: 'Submit',
                      color: context.primaryColor,
                      onTap: () => finish(ctx, true),
                    ),
                  ],
                ),
              ],
        ...
        try {
          await createRefundRequest(
            ...
            reason: reasonCont.text.trim().isEmpty ? null : reasonCont.text.trim(),
          );
          toast('Refund request submitted. It will be reviewed by the administrator.');
          init();
          setState(() {});
        } catch (e) {
          toast(e.toString());
