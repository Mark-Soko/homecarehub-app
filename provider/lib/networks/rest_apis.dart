...
Future<num> getUserWalletBalance() async {
  var res = await handleResponse(await buildHttpResponse('user-wallet-balance', method: HttpMethodType.GET));
  if (res is Map) {
    if (res['data'] != null && res['data'] is Map) return (res['data'] as Map)['balance'] ?? 0;
    return (res['balance'] ?? 0).toDouble();
  }
  return 0;
}

/// Tuma wallet: public config (limits, currency). No auth required.
Future<Map<String, dynamic>> getWalletConfig() async {
  var res = await handleResponse(await buildHttpResponse('config', method: HttpMethodType.GET));
  return Map<String, dynamic>.from(res is Map ? res : {});
}

/// Tuma wallet: deposit via M-Pesa STK. Balance updated when webhook is received.
Future<BaseResponseModel> walletDepositTuma({required double amount, required String phone, required String idempotencyKey}) async {
  Map<String, String> headers = buildHeaderTokens();
  headers['Idempotency-Key'] = idempotencyKey;
  var res = BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse(
    'wallet/deposit',
    method: HttpMethodType.POST,
    request: {'amount': amount, 'phone': phone},
    header: headers,
  )));
  return res;
}

/// Tuma wallet: withdraw to M-Pesa (B2C).
Future<BaseResponseModel> walletWithdrawTuma({required double amount, required String phone, required String idempotencyKey}) async {
  Map<String, String> headers = buildHeaderTokens();
  headers['Idempotency-Key'] = idempotencyKey;
...
/// Tuma wallet: internal transfer to another user by phone.
Future<BaseResponseModel> walletTransferTuma({required double amount, required String recipientPhone, required String idempotencyKey}) async {
  Map<String, String> headers = buildHeaderTokens();
  headers['Idempotency-Key'] = idempotencyKey;
  return BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse(
    'wallet/transfer',
    method: HttpMethodType.POST,
    request: {'amount': amount, 'recipient_phone': recipientPhone},
    header: headers,
  )));
}

/// Statutory deduction types (optional voluntary_only for dropdown).
Future<List<Map<String, dynamic>>> getStatutoryDeductionTypes({bool voluntaryOnly = true}) async {
  var res = await handleResponse(await buildHttpResponse(
    'statutory-deduction-types?voluntary_only=${voluntaryOnly ? 1 : 0}',
    method: HttpMethodType.GET,
  ));
  if (res is List) return List<Map<String, dynamic>>.from(res.map((e) => Map<String, dynamic>.from(e as Map)));
  if (res is Map && res['data'] != null) return List<Map<String, dynamic>>.from((res['data'] as List).map((e) => Map<String, dynamic>.from(e as Map)));
  return [];
}

/// Pay voluntary statutory for the month from wallet.
Future<BaseResponseModel> voluntaryStatutoryPay({required int statutoryDeductionTypeId, required double amount, required String yearMonth}) async {
  return BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse(
    'voluntary-statutory-pay',
    method: HttpMethodType.POST,
    request: {
...
/// List current user's voluntary statutory payments.
Future<List<Map<String, dynamic>>> getVoluntaryStatutoryPayments({String? yearMonth}) async {
  String url = 'voluntary-statutory-payments';
  if (yearMonth != null && yearMonth.isNotEmpty) url += '?year_month=$yearMonth';
  var res = await handleResponse(await buildHttpResponse(url, method: HttpMethodType.GET));
  if (res is List) return List<Map<String, dynamic>>.from(res.map((e) => Map<String, dynamic>.from(e as Map)));
  if (res is Map && res['data'] != null) return List<Map<String, dynamic>>.from((res['data'] as List).map((e) => Map<String, dynamic>.from(e as Map)));
  return [];
}

/// Submit a refund request for a paid booking. Requires admin approval.
Future<Map<String, dynamic>> createRefundRequest({required int bookingId, String? reason, double? amount}) async {
  var body = <String, dynamic>{'booking_id': bookingId};
  if (reason != null && reason.isNotEmpty) body['reason'] = reason;
  if (amount != null && amount > 0) body['amount'] = amount;
  var res = await handleResponse(await buildHttpResponse('refund-request', method: HttpMethodType.POST, request: body));
  return Map<String, dynamic>.from(res is Map ? res : {'message': 'OK'});
}

/// List current user's refund requests.
Future<List<Map<String, dynamic>>> getRefundRequests() async {
  var res = await handleResponse(await buildHttpResponse('refund-requests', method: HttpMethodType.GET));
  if (res is List) return List<Map<String, dynamic>>.from(res.map((e) => Map<String, dynamic>.from(e as Map)));
  if (res is Map && res['data'] != null) return List<Map<String, dynamic>>.from((res['data'] as List).map((e) => Map<String, dynamic>.from(e as Map)));
  return [];
}
...
Future<List<PaymentSetting>> getPaymentGateways({bool isAdvancePayment = false, bool requireCOD = true, bool requireWallet = true}) async {
  try {
    ...
    // Limit to Kenyan context: Cash, Wallet, and M-Pesa
    res.removeWhere((element) => ![PAYMENT_METHOD_COD, PAYMENT_METHOD_FROM_WALLET, PAYMENT_METHOD_MPESA].contains(element.type));
...
// region Resource API (Knowledge Hub)

Future<List<ResourceData>> getResourceListAPI({
  int? page,
  required List<ResourceData> resourceData,
  Function(bool)? lastPageCallback,
  required String audience,
}) async {
  try {
    ResourceListResponse res = ResourceListResponse.fromJson(
      await handleResponse(
        await buildHttpResponse('resource-list?audience=$audience&per_page=$PER_PAGE_ITEM&page=$page', method: HttpMethodType.GET),
      ),
    );

    if (page == 1) resourceData.clear();

    resourceData.addAll(res.data.validate());
...
Future<ResourceData> getResourceDetailAPI({
  ...
  try {
    ...
    Map<String, dynamic> res = await handleResponse(
      await buildHttpResponse('resource-detail', request: request, method: HttpMethodType.POST),
    );

    final detail = res['resource_detail'];
    return ResourceData.fromJson(detail is Map ? Map<String, dynamic>.from(detail) : <String, dynamic>{});
  } catch (e) {
    appStore.setLoading(false);
    throw e;
  }
