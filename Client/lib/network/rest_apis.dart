...
Future<void> initiateMpesaPayment({
  ...
  /// Create/update payment record for this booking with type 'mpesa'
  Map<String, dynamic> methodReq = {
    CommonKeys.bookingId: bookingId,
    CommonKeys.paymentMethod: PAYMENT_METHOD_MPESA,
  };

  await handleResponse(await buildHttpResponse('get-payment-method', request: methodReq, method: HttpMethodType.POST));

  /// Trigger STK push
  Map<String, dynamic> mpesaReq = {
    CommonKeys.bookingId: bookingId,
    'phone': phone,
  };

  await handleResponse(await buildHttpResponse('mpesa/stk-initiate', request: mpesaReq, method: HttpMethodType.POST));
}
...
Future<List<PaymentSetting>> getPaymentGateways({bool requireCOD = true, bool requireWallet = true, bool isAddWallet = false}) async {
  ...
  try {
    ...
    /// Hard-limit to Kenyan context: only Cash, Wallet, and M-Pesa
